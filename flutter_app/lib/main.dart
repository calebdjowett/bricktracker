...
class AnalyticsScreen extends StatelessWidget { const AnalyticsScreen({super.key, required this.api}); final BrickTrackerApi api; @override Widget build(BuildContext context) => FutureBuilder<List<CollectionItem>>(future: api.collection(), builder: (context, snapshot) { if (!snapshot.hasData) return const Center(child: CircularProgressIndicator()); final items = snapshot.data!; final spots = [for (var i = 0; i < items.length; i++) FlSpot(i.toDouble(), items[i].currentValueCents / 100)]; return ListView(padding: const EdgeInsets.all(16), children: [Text('Portfolio growth', style: Theme.of(context).textTheme.titleLarge), SizedBox(height: 220, child: LineChart(LineChartData(lineBarsData: [LineChartBarData(spots: spots, isCurved: true, color: Colors.teal)], titlesData: const FlTitlesData(show: false), borderData: FlBorderData(show: false))), const SizedBox(height: 24), Text('Set appreciation', style: Theme.of(context).textTheme.titleLarge), ...items.map((item) => ListTile(title: Text(item.name), trailing: Text('${item.gainLossPercent?.toStringAsFixed(1) ?? '0.0'}%')))]); }); }